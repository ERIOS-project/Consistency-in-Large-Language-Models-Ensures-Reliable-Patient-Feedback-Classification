# This code create the three dimensional array to describe the GPT4+ICA, GPT4+ECA and GPT4+GCA classifications from the multiple txt files generated by each agent.

# Store the possible agents
agent = c("1","2","3")

# Create the array squeleton
matrix_llm_CA = array(data=0, dim=c(
  length(index),
  length(category),
  length(tone),
  length(agent)
...
llm_uncleaned_folders = paste("data/GPT4/gpt4-lot",agent,"/",sep="")



# Txt files generated by GPT4 might not be a valid json file. We need to read every line of it to convert it to valid json files.

for(current_agent in 1:length(agent)){
  
  print(paste("----------------------- cleaning llm",agent[current_agent],"-----------------------------"))
  ...
  # Defining the unclean txt files storage and the clean storage.
  llm_uncleaned_folder_1 = paste(llm_uncleaned_folders[current_agent],"llm_output_1/prompt_2/",sep="")  
  llm_uncleaned_folder_2 = paste(llm_uncleaned_folders[current_agent],"llm_output_2/prompt_2/",sep="") 
  clean_folder_1 = paste(llm_uncleaned_folders[current_agent],"clean_output_1/",sep="")
  clean_folder_2 = paste(llm_uncleaned_folders[current_agent],"clean_output_2/",sep="")
  
  for(folder in 1:2){ #iterates through each independant run of GPT4
  ...
  print(paste("----------------------- Consistency assessment of agent ",agent[current_agent],"-----------------------------"))
  
  # -------------------------------------------------------------------------------------------------------------
  #step 2 : gather the confidence scores
  # -------------------------------------------------------------------------------------------------------------
  
  # Store the possible standalone agents
  sa_agent = c("LLM1","LLM2")
  ...
  dimnames = list(index,category,tone,sa_agent))
  ...
  for(i in 1:length(index)){
    ...
    for(current_sa_agent in sa_agent){
      suppressWarnings({
        input = paste(trimws(readLines(file_correspondance[[current_sa_agent]][i])),collapse=" ")
      })
        ...
        for(current_tone in tone){
          if(current_tone=="positive" & 
            ...
            matrix_class[i,which(category %in% current_category),tone=="positive",
                         sa_agent=current_sa_agent] = 1
            matrix_justif[i,which(category %in% current_category),tone=="positive",
                          sa_agent=current_sa_agent] = input_2[[current_category]][[1]]
          }
          if(current_tone=="negative" & 
            ...
            matrix_justif[i,which(category %in% current_category),tone=="negative",
                          sa_agent=current_sa_agent] = input_2[[current_category]][[2]]
          }
        }
      }
    }
  ...
  for(i in 1:length(index)){
    for(current_category in category){
      for(current_tone in tone){
        ...
        current_justification1 = matrix_justif[i,category==current_category,tone==current_tone,sa_agent=="LLM1"]
        current_justification2 = matrix_justif[i,category==current_category,tone==current_tone,sa_agent=="LLM2"]
        ...
        # score : class1
        if(matrix_class[i,category==current_category,tone==current_tone,sa_agent=="LLM1"]==1){
          matrix_confidence[i,category==current_category,tone==current_tone,scores=="class1"] = 1
        }
        # score : class2
        if(matrix_class[i,category==current_category,tone==current_tone,sa_agent=="LLM2"]==1){
          matrix_confidence[i,category==current_category,tone==current_tone,scores=="class2"] = 1
        }
        # score : syntax1  (as few typos occur without comprometting the meaning of the implication, only 20 first characters are checked)
  ...
  matrix_llm_CA[,,,current_agent] = matrix_confidence
}


saveRDS(matrix_llm_CA,"data/GPT4/matrix_GPT4_CA.rds")