...
verbatims = openxlsx::read.xlsx("data/sample/verbatims.xlsx")$verbatim
...
# # To avoid this very expensive calculus multiple times, I saved it in nmf_result.RDS
# nmf_result <- NMF::nmf(dtm_df, rank = k, method = "snmf/r", nrun = 5)
# saveRDS(nmf_result, "E:/Git/Projects/Verbatims/10_Article/02_Analyse_secondaire/04_NB/NMF_result/nmf_result.RDS")
nmf_result = readRDS("E:/Git/Projects/Verbatims/10_Article/02_Analyse_secondaire/04_NB/NMF_result/nmf_result.RDS")
...
for(i in 1:ncol(folds)){
  ...
  for(current_category in 1:length(category)){
    for(current_tone in 1:length(tone)){
      ...
      current_model = naiveBayes(x = W[other_folds,] ,y =matrixGS[other_folds,current_category,current_tone],laplace = 3 )
      
      if(length(table(matrixGS[other_folds,current_category,current_tone]))==1){
