# This code takes in entry llm txt output, clean them and store the GPT4 +/-CA in R object matrices
...
llm_uncleaned_folder_1 = "data/GPT4/GPT4_output_1/"
llm_uncleaned_folder_2 = "data/GPT4/GPT4_output_2/"
clean_folder_1 = "data/GPT4/GPT4_clean_output_1/"
clean_folder_2 = "data/GPT4/GPT4_clean_output_2/"
...
for(i in 1:length(index)){
  for(current_category in category){
    for(current_tone in tone){
      ...
      # score : syntax1
      if(substr(element1,1,20) %in% substr(unlist(classification_rules[[current_category]]),1,20) ){
        matrix_confidence[i,category==current_category,tone==current_tone,scores=="syntax1"] = 1
      }
      # score : syntax2
      if(substr(element2,1,20) %in% substr(unlist(classification_rules[[current_category]]),1,20) ){
